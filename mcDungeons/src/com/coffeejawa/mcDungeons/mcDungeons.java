...
import com.coffeejawa.LootChests.LootChestsMain;
...
public class mcDungeons extends JavaPlugin {
	...
	public LootChestsMain lootchests;
	public boolean bLootChestsEnabled;
	...
	public void onEnable() 
	{
        ...
        if(getConfig().getBoolean("useWorldGuard")){
            ...
            if (worldguard == null) {
                logger.info("WorldGuard plugin not found");
                bWorldGuardEnabled =  false;
            }
            else {
        ...
        // check for LootChests
        lootchests = (LootChestsMain) getServer().getPluginManager().getPlugin("LootChests");
        if (lootchests == null)
        {
            logger.info("LootChests plugin not found");
            bLootChestsEnabled =  false;
        }
        else {
            logger.info("[" + this.getDescription().getName() +  "] LootChests support enabled.");
            bLootChestsEnabled = true;
        }                
	...
	public void onDisable(){
        ...
        logger.info("mcDungeons has been disabled.");
	}
